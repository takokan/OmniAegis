...
export default function VersionRegistry() {
  ...
  const getStatusBadge = (status: string) => {
    const colors: Record<string, string> = {
      ...
      Previous: 'bg-surface-elevated text-text-secondary',
    };
  ...
  return (
    <div className="space-y-4">
      {data.versions.map((version) => (
        <div
          key={version.id}
          className="premium-card rounded-3xl p-6 hover:shadow-md transition"
        >
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex-1 space-y-2">
              <div className="flex items-center gap-3">
                <h3 className="text-lg font-bold text-text-primary">
                  {version.type} <span className="font-mono text-sm text-text-secondary">{version.version}</span>
                </h3>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {Object.entries(version.performanceMetrics).map(([key, value]) => (
                  <span key={key} className="rounded-full bg-surface-elevated px-3 py-1 text-xs text-text-secondary">
            ...
            <div className="flex-shrink-0 flex gap-2">
              {version.status === 'Previous' && (
                <button
                  ...
                  className="rounded-2xl border border-border-default bg-surface-primary hover:bg-surface-elevated disabled:bg-surface-elevated text-text-primary font-semibold py-2 px-4 text-sm transition"
                >
                  {rollingBack === version.id ? 'Rolling back...' : 'Rollback'}
                </button>
              )}
              {version.status === 'Shadow' && (
                <button className="rounded-2xl border border-border-default bg-surface-primary hover:bg-surface-elevated text-text-primary font-semibold py-2 px-4 text-sm transition">
                  Compare
                </button>
              )}
              <button className="rounded-2xl border border-border-default bg-surface-primary hover:bg-surface-elevated text-text-primary font-semibold py-2 px-4 text-sm transition">
